...
): Promise<ICompetition | undefined> {
  // fetch competition details from the backend
  const accessToken = await AsyncStorage.getItem("accessToken");

  try {
    const response = await fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/competition/${competitionId}`,
      {
        method: "GET",
        headers: {
    ...
    if (process.env.DEBUG) {
      console.log(
        `[debug] fetched competition details: ${JSON.stringify(
          competitionData
        )}`
      );
    }
...
export async function fetchUserCompetitions() {
  ...
  try {
    const response = await fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/competition`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (process.env.DEBUG) {
      console.log(
        `[debug] fetched competitions: ${JSON.stringify(competitionData)}`
      );
    }
    return competitionData;
  } catch (error) {
...
export async function createCompetition(competitionFormData: ICompetition) {
  ...
  try {
    ...
    if (process.env.DEBUG) {
      console.log(
        `[debug] created competition: ${JSON.stringify(competitionFormData)}`
      );
    }
  } catch (error) {
...
export async function joinCompetition(competitionId: number) {
  ...
  try {
    await fetch(
      `${process.env.EXPO_PUBLIC_API_URL}/competition/${competitionId}/join/`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
